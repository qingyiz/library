...
#include"string.h"
#include"stdlib.h"
#define  LIBRARIAN_MAX  2 /*管理员人数*/
int N;
int book_num_now = 0;  /*记录当前结构体中有多少书籍*/ 
int login_num;
int login_ID;
char login_mima[20];
/**********定义图书结构体类型book*******/
...
{ 
    int i;
    int book_num_updata = book_num_now; /*用于记录添加书籍之前的书籍数量*/
    system("CLS");
    printf("\t\t         =====================        \n");
    printf("\t\t               录入图书信息              \n");
    printf("\t\t         =====================        \n\n");
...
printf("\n");
    ...
    while(1)
    {
	if(strcmp(temp_mima,login_mima)==0)/*比较密码*/
	{
		printf("\t           请输入新密码:");
		scanf("%s",temp1);
		printf("\t           请再输入一次:");
...
void show_all_book()
{
	...
	{
		printf("\t编号    图书名称         作者             出版社         出版时间       位置  总数量  借阅数\n");
		for(i=0;i<book_num_now;i++)
		{
			printf("\t%-7d %-16s %-16s %-12s   %4d年%2d月%2d日 %-7s %-7d %-4d\n",bk[i].num,
																 				 bk[i].name,
																 				 bk[i].writer,
																 				 bk[i].publish,
																 				 bk[i].t.year,
...
void weijieyue()
{
	...
	else
	{
	    for(i=0;i<book_num_now;i++)
			...
			{
				if(k==0)
				    printf("\t编号    图书名称         作者             出版社         出版时间       位置  总数量  剩余数\n");
            printf("\t%-7d %-16s %-16s %-12s   %4d年%2d月%2d日 %-7s %-7d %-4d\n",bk[i].num,
																 				 bk[i].name,
																 				 bk[i].writer,
																 				 bk[i].publish,
...
void show()
{
	...
	do
	{
    	 ...
    	 switch(x)
    	 {
        	 ...
        	 case 3:system("cls");showbook_publish();getch();break;	/*按出版社查看*/
       	   	 case 4:system("cls");weijieyue();getch();break;		/*查看未借阅图书*/
       	   	 case 5:system("cls");yijieyue();getch();break;			/*查看已借阅图书*/
		 	 case 6:system("cls");show_all_book();getch();break;	/*查看所有图书*/	
    	 }                                    
	}while(x!=7);
}
/*********按书名借阅*******/
void jie_name()
{
    ...
    scanf("%s",name);
    for(i=0;i<book_num_now;i++)
        if(strcmp(bk[i].name,name)==0&&((bk[i].total-bk[i].jie)!=0))/*找到图书并确认图书没有被借出，记录图书下标*/
		{
		    book_xb=i;
			k++;
			break;
	...
	if(k==0)
	{
		printf("\t不存在该书，或该书已经全部借出!请正确输入图书名称!\n\t\t按任意键返回...");
		getch();
        break;
	}
	if(k==1)
				   ...
				   {				    
						if(s==1)
						{
							...
							switch(x2)
							{
								...
								case 5:system("cls");xinxi_fromFILE();getch();break;	/*从文件中录入信息*/
							}	
						}
						else
						{
